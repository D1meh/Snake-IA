...
class Window:
	...
	def draw(self, game: Game):
		for x in range(0, (self.training.size + 2) * 50, 50):
			for y in range(0, (self.training.size + 2) * 50, 50):
				...
				if cell == '0': # Empty
					pygame.draw.rect(self.SCREEN, "grey", (x + WIDTH_OFFSET, y, 50, 50))
				elif cell == 'H': # Head
					pygame.draw.rect(self.SCREEN, "yellow", (x + WIDTH_OFFSET, y, 50, 50))
				elif cell == 'S': # Body segment
					pygame.draw.rect(self.SCREEN, "blue", (x + WIDTH_OFFSET, y, 50, 50))
				elif cell == 'R': # Red apple
					pygame.draw.rect(self.SCREEN, "red", (x + WIDTH_OFFSET, y, 50, 50))
				elif cell == 'G': # Green apple
					pygame.draw.rect(self.SCREEN, "green", (x + WIDTH_OFFSET, y, 50, 50))
				else: # Wall
					pygame.draw.rect(self.SCREEN, "black", (x + WIDTH_OFFSET, y, 50, 50))

				pygame.draw.rect(self.SCREEN, "black", (x + WIDTH_OFFSET, y, 50, 50), 1)

	def drawText(self, game, duration, lastDirection):
		firstLine = "Score: " + str(game.snake.size)
		#secondLine = "Coordinates: " + str(game.snake.getHead())
	...
	def drawNerdText(self, game, state):
		...
		for i, line in enumerate(content):
			if i == 0:
				...
				firstPartRect.topleft = (screenWidth - 250, screenHeight / 5 * i)
				secondPartRect.topleft = (screenWidth - 250, screenHeight / 5 * i + 30)
				thirdPartRect.topleft = (screenWidth - 250, screenHeight / 5 * i + 60)
		...
		for i, qValue in enumerate(qValues):
			...
			qValueText = font.render(direction + ": " + str(qValue), True, "green" if qValue == max(qValues) else "white")
			qValueRect = qValueText.get_rect()
			qValueRect.topleft = (screenWidth - 250, screenHeight - 100 + i * 30)
	...
	def run(self):
		...
		while True:
			...
			game = Game(self.training.size, True)
			state = State(game)
			currentDuration = 0
