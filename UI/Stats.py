...
class Stats:
	...
	def updateStats(sizes, durations, gridSize, sessions):
		...
		with open(STATS_FILE, "r") as f:
			content = json.load(f)
			for item in content:
				if item["size"] == gridSize and item["trainingSessions"] == sessions:
					stats = item
					break
			if stats is None:
				stats = TEMPLATE.copy()
	...
	def showStat(screen, clock, stats):
		...
		while True:
			...
			for key, value in texts.items():
				...
				valueTextRect.topleft = (WIDTH_OFFSET + len(key) * 20, 250 + idx * 30)
	...
	def showAllStats(screen, clock):
		...
		while True:
			...
			for event in pygame.event.get():
				...
				if event.type == pygame.MOUSEBUTTONDOWN:
					...
					if statToDisplay is not None:
						Stats.showStat(screen, clock, content[indexToStart + statToDisplay])
			...
			for idx in range(3):
				...
				text = f"Grid Size: {stat['size']}, Sessions: {stat['trainingSessions']}"
			...
			for idx, text in enumerate(buttonsText):
				...
				buttonText = buttonsFont.render(text, True, buttonColor)
				buttonTextRect = buttonText.get_rect()
				buttonTextRect.topleft = (100 + (10 - len(text)) * 10 + 400 * idx, 812)
